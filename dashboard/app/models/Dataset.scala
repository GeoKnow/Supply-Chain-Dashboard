...
object Dataset {
   ...
   */
  def retrieveDelivery(id: String): Delivery = queryDeliveries(Some(id)) match {
   ...
   */
  def retrieveDeliveries(): Seq[Delivery] = queryDeliveries(None)

  /**
   * Retrieves all addresses.
   */
  def retrieveAddresses(): Seq[Address] = {
    // Build query string
    val queryStr =
      """
